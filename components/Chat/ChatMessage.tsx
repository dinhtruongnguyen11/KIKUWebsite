...
export const ChatMessage: FC<Props> = memo(
  ({ message, messageIndex, onEdit }) => {
    ...
    return (
      <div
        className={`group justify-center items-center flex md:px-4
        my-8 rounded-xl mx-5 z-0`}
        style={{ overflowWrap: 'anywhere' }}
      >
        <div className="md:max-w-2xl flex justify-center items-center w-full gap-2">
          <div className="min-w-[40px]">
            {message.role === 'assistant' ? (
              <Image src="/kikulg2.ico" width={30} height={30} alt="" />
            ) : (
              <img
          ...
          <div
            className={`relative w-full rounded-r-xl border-l-4 m-auto flex p-4 
          md:gap-6 md:py-6  ${
            message.role === 'assistant'
              ? ' text-gray-800 bg-white shadow  border-[#2AAAE3]'
              : ' text-gray-800 bg-white shadow border-[#78909C]'
          }`}
          >
                ...
                <div className="flex w-full">
                  {isEditing ? (
                    <div className="flex w-full flex-col">
                      <textarea
                        ref={textareaRef}
                        className="outline-none w-full resize-none whitespace-pre-wrap 
                        border-none rounded-lg p-2"
